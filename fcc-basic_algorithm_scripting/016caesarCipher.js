...
// My Solution (with some inspiration from https://www.selftaughtjs.com/algorithm-sundays-breaking-a-rot-13-caesar-cipher/):
...
function rot13(str) { // LBH QVQ VG!
  ...
  for (var i = 0; i < str.length; i++) {
    ...
    if (currentPosition < 65 || currentPosition > 90) {
      decrypted.push(String.fromCharCode(currentPosition));
    } else if (shiftedPosition > 90) {
      decrypted.push(String.fromCharCode(64 + currentPosition - 90 + 13));
    } else {
      decrypted.push(String.fromCharCode(shiftedPosition));
    }
  }
...
If the character is not uppercase English alphabets(i.e. its ascii doesn’t lie between 65 and 91 ), we’ll leave it as it is and continue48 with next iteration.
...
Relevant Links
Array.prototype.map98
String.prototype.split8
Array.prototype.join7
*/
...
Now let us consider mapping a range of 26 numbers i.e. between [65 - 90] (which represents uppercase English alphabets12 in Unicode character set32) to a range of numbers between [0 - 25].
...
[Z]  90 % 26 ⇔ 12 + 65 =  77 [M]
Code Explanation:
String.prototype.replace function28 lets you transform a String based on some pattern match (defined by a regular expression), and the transformation function41 (which is applied to each of the pattern matches).
Arrow function119 syntax is used to write the function parameter to replace().
